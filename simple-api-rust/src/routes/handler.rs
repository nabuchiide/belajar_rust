use chrono::prelude::*;
use uuid::Uuid;
use warp::{http::StatusCode, Reply, reply::json, reply::with_status};
use crate::routes::model::{DB, QueryOption};
use crate::routes::model::{Todo, UpdateTodoSchema};
use crate::routes::response::{GenericResponse, SingleTodoResponse, TodoData, TodoListResponse};
...
pub async fn todos_list_handler(opts: QueryOption, db: DB) -> WebResult<impl Reply> {
...
pub async fn get_todo_handler(id: String, db: DB) -> WebResult<impl Reply> {
    ...
    for todo in vec.iter() {
        if todo.id == Some(id.to_owned()) {
            ...
            return Ok(with_status(json(&json_response), StatusCode::NOT_FOUND));
        }
    }
...
pub async fn edit_todo_handler(
    ...
    for todo in vec.iter_mut() {
        if todo.id == Some(id.clone()) {
            ...
            let payload = Todo {
                id: todo.id.to_owned(),
                title: if !content.is_empty() { title } else { todo.title.to_owned() },
                content: if !content.is_empty() { content } else { todo.content.to_owned() },
                completed: if body.completed.is_some() { body.completed } else { todo.completed },
